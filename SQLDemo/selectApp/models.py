...
class Products(models.Model):
    ...
    def __str__(self):
        return f'{self.productcode} :: {self.productname} :: {self.productline} :: {self.productscale}\
                 :: {self.productvendor} :: {self.productdescription}  :: {self.quantityinstock} :: {self.buyprice} :: {self.msrp}'

    '''
    What is use of __str__ method in models.py?
    
    THis is my current output with __str__
    -----------------------------------------
    >>> from selectApp.models import Products
    >>> p = Products.objects.all()[:1]
    >>> p
    <QuerySet [<Products: S10_1678 :: 1969 Harley Davidson Ultimate Chopper :: Productlines object (Motorcycles) :: 1:10                 :: Min Lin Diecast :: This replica features working kickstand,
     front suspension, gear-shift lever, footbrake lever, drive chain, wheels and steering. All parts are particularly delicate due to their precise scale and require special care and attention.  ::
    7933 :: 48.81 :: 95.70>]>
    
    comment __str__ & run same code in shell
    ----------------------------------------
    restart shell because we have made change in model.py
    exit()
    
    This is the output 
    <QuerySet [<Products: Products object (S10_1678)>]>
    
    
    so ,, if we dont use __str__ then we dont get readable result,,,
    we get result in the form of object
